...
function ModalRemote(modalId) {
    ...
    function successRemoteResponse(response) {
        // Close modal if response contains close field
        if (response.close !== undefined && response.close) {
            var that = this;
            setTimeout(function(){
                that.hide();
                if(response.code == '0'){//后台执行成功
                    setTimeout(function(){
                        if(localStorage.getItem("show-message") == '1'){
                            swal({
                                title: "操作成功",
                            ...
                            toastr.success('操作成功，'+response.message);
                        }
                        setTimeout(function(){
        ...
        if (response.footer !== undefined){
            ...
            this.setFooter('<button type="button" class="btn btn-white" data-dismiss="modal">取  消</button><button type="submit" class="btn btn-primary">确  认</button>');
        }
     ...
     *   - close            (boolean remote close modal)
     ...
     * @params {elm}
     */
    this.open = function (elm, ids , rowData) {
        console.log(ids,rowData);
        /**
        ...
        if ($(elm).hasAttr('data-confirm-title') || $(elm).hasAttr('data-confirm-message')) {
            ...
            if($(elm).attr('data-get-field')){
                ids = $(elm).attr('data-get-field')+'='+ids;
            }
            this.doRemote(
                $(elm).hasAttr('href') ? $(elm).attr('href') : $(elm).attr('data-url'),
                $(elm).hasAttr('data-request-method') ? $(elm).attr('data-request-method') : 'GET',
